...
use cursive::{
    ...
    views::{Button, Dialog, DummyView, LinearLayout, OnEventView, SelectView, TextView, EditView },
    Cursive,
};
        ...
        OnEventView::new(
            Dialog::around(
                ...
                .child(EditView::new().on_submit(|s, item| {
                    s.with_user_data(|data: &mut data::ProgramData| {
                        data.user_list.insert(item.to_string(), User::new(item.to_string()))
                    });
                    s.pop_layer();
                    s.pop_layer();
                    user_list(s);
    ...
    siv.add_layer(
        OnEventView::new(
            Dialog::around(
                ...
                .child(DummyView)
                .child(EditView::new().on_submit(|s, item| {
                    let selected = s.call_on_name("user_select", |view: &mut SelectView| {
                        let (slug, _user) = view
                            .get_item(
                                view.selected_id().expect("cannot get the ID of the selected item"),
                            )
...
fn remove_user(siv: &mut Cursive) {
    let selected = siv.call_on_name("user_select", |view: &mut SelectView| {
        let (slug, _user) = view.get_item(view.selected_id().expect("cannot get selected id")).expect("cannot get selected item");
    ...
    siv.add_layer(
        OnEventView::new(
            Dialog::around(TextView::new(format!("You are going to delete {}", &selected)))
                .title("Delete a user")
                .button("Yes", move |s| {
                    s.with_user_data(|data: &mut data::ProgramData| {
